...
aictab(mod_list_lambda) #  ~ trait/elevationScaled*sex        #mod9
```
...
```{r}
# Now the fitted models (already assembled into a list) can be compared with AICc
aictab(mod_list_OU)   #  ~ trait/Habitat, but note OU models are worse than lambda models
```
...
**Simon left off working here (am still diagnosing strange values for female tail slope trend):**
